module Administrate
  class ApplicationController < ActionController::Base
    ...
    def index
      ...
      search_term = (params[:search].blank? || params[:search].is_a?(String) ? params[:search] : params[:search][:search]).to_s.strip
      resources = Administrate::Search.new(scoped_resource,
                                           dashboard_class,
                                           search_term).run

      filters = params[:search].blank? || params[:search].is_a?(String) ? {} : params[:search].except(:search)
      advanced_search = Administrate::AdvancedSearch.new(resources,
                                                         dashboard_class,
                                                         filters)
      resources = advanced_search.run

      resources = apply_collection_includes(resources)
      resources = order.apply(resources)
      resources = resources.page(params[:_page]).per(records_per_page)
      page = Administrate::Page::Collection.new(dashboard, order: order)
      advanced_search.page = page
